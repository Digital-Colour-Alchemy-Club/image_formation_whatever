from image_formation_toolkit._vendor.aenum import (
    Enum,
    IntEnum,
    Flag,
    UniqueEnum,
...
class TestEnumV3(TestCase):
    ...
    def test_autonumber_with_irregular_values(self):
        ...
        with self.assertRaisesRegex(
            TypeError, "number of fields provided do not match init"
        ):
    ...
    if pyver == 3.4:
        ...
        def test_class_nested_enum_and_pickle_protocol_four(self):
            ...
            test_pickle_exception(
                self.assertRaises, PicklingError, self.NestedEnum.twigs, protocol=(0, 3)
            )
            test_pickle_dump_load(
                self.assertTrue, self.NestedEnum.twigs, protocol=(4, HIGHEST_PROTOCOL)
            )
        ...
        def test_class_nested_enum_and_pickle_protocol_four(self):
            ...
            self.NestedEnum.__qualname__ = "%s.NestedEnum" % self.__class__.__name__
            test_pickle_dump_load(
                self.assertTrue, self.NestedEnum.twigs, protocol=(0, HIGHEST_PROTOCOL)
            )
    ...
    if pyver >= 3.4:
        ...
        def test_subclasses_with_getnewargs_ex(self):
            ...
            test_pickle_dump_load(
                self.assertEqual, NI5, 5, protocol=(4, HIGHEST_PROTOCOL)
            )
            self.assertEqual(NEI.y.value, 2)
            test_pickle_dump_load(
                self.assertTrue, NEI.y, protocol=(4, HIGHEST_PROTOCOL)
            )
