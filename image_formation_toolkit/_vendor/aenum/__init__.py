...
def _is_internal_class(cls_name, obj):
    ...
    if pyver < 3.3:
        ...
        return (
            not _is_descriptor(obj)
            and qualname
            and re.search(r"\.?%s\.\w+$" % cls_name, qualname)
        )
    ...
    def __delattr__(cls, attr):
        ...
        if NamedConstant is not None and isinstance(cur_obj, NamedConstant):
            raise AttributeError(
                "cannot delete constant <%s.%s>"
                % (cur_obj.__class__.__name__, cur_obj._name_)
            )
...
temp_constant_dict = {}
temp_constant_dict["__doc__"] = (
    "NamedConstants protection.\n\n    Derive from this class to lock NamedConstants.\n\n"
)
...
class NamedTupleMeta(type):
    ...
    def __new__(metacls, cls, bases, clsdict, size=undefined):
        ...
        for k, v in original_dict.items():
            if k not in original_dict._field_names:
                ...
                if isinstance(v, baseinteger):
                    ...
                    isinstance(v, tuple)
                    and len(v) in (2, 3)
                    and isinstance(v[0], baseinteger)
                    and isinstance(v[1], (basestring, NoneType))
                ):
                    ...
                    isinstance(v, tuple)
                    and len(v) in (1, 2)
                    and isinstance(v[0], (basestring, NoneType))
                ):
                    ...
                    raise ValueError(
                        "not sure what to do with %s=%r (should be OFFSET [, DOC [, DEFAULT]])"
                        % (k, v)
                    )
                target[k] = v
        ...
        if not _order_:
            if unnumbered:
                raise ValueError(
                    "_order_ not specified and OFFSETs not declared for %r"
                    % (unnumbered.keys(),)
                )
            for name, (index, doc, default) in sorted(
                numbered.items(), key=lambda nv: (nv[1][0], nv[0])
            ):
                if index in seen:
                    aliases.append(name)
                else:
            ...
            if missing:
                raise ValueError(
                    "unable to order fields: %s (use _order_ or specify OFFSET"
                    % missing
                )
            offsets = OrderedDict()
            ...
            for k, (index, doc, default) in sorted(
                numbered.items(), key=lambda nv: (nv[1][0], nv[0])
            ):
                if index in seen:
                    aliases.append(k)
        ...
        assert len(fields) + len(aliases) == len(
            offsets
        ), "number of fields + aliases != number of offsets"
        assert set(fields) & set(offsets) == set(
            fields
        ), "some fields are not in offsets: %s" % set(fields) & set(offsets)
        assert set(aliases) & set(offsets) == set(
            aliases
        ), "some aliases are not in offsets: %s" % set(aliases) & set(offsets)
        for name, (index, doc, default) in offsets.items():
            assert isinstance(
                index, baseinteger
            ), "index for %s is not an int (%s:%r)" % (name, type(index), index)
            assert (
                isinstance(doc, (basestring, NoneType)) or doc is undefined
            ), "doc is not a str, None, nor undefined (%s:%r)" % (name, type(doc), doc)
        ...
        namedtuple_class = super(NamedTupleMeta, metacls).__new__(
    ...
    def _convert_fields(*namedtuples):
        ...
        for cls in namedtuples:
            ...
            for field in cls._fields_:
                ...
                all_fields.append(
                    (field, base + desc.index, desc.__doc__, desc.default)
                )
        return all_fields
    ...
    def __call__(cls, *args, **kwds):
        ...
        if cls is NamedTuple:
            ...
            try:
                ...
                raise TypeError(
                    "too few arguments to NamedTuple: %s, %s"
                    % (original_args, original_kwds)
                )
            if args or kwds:
                raise TypeError(
                    "too many arguments to NamedTuple: %s, %s"
                    % (original_args, original_kwds)
                )
            if pyver < 3.0:
...
def __new__(cls, *args, **kwds):
    if cls._size_ is TupleSize.fixed and len(args) > cls._defined_len_:
        raise TypeError(
            "%d fields expected, %d received" % (cls._defined_len_, len(args))
        )
...
def __repr__(self):
    if len(self) == len(self._fields_):
        return "%s(%s)" % (
            ...
            ", ".join(["%s=%r" % (f, o) for f, o in zip(self._fields_, self)]),
        )
    else:
...
def __str__(self):
    return "%s(%s)" % (
        self.__class__.__name__,
        ", ".join(["%r" % (getattr(self, f),) for f in self._fields_]),
    )
...
class EnumConstants(NamedConstant):
    AutoValue = constant(
        "autovalue", "values are automatically created from _generate_next_value_"
    )
    AutoNumber = constant(
        "autonumber", "integer value is prepended to members, beginning from START"
    )
...
class enum(object):
    ...
    def __repr__(self):
        ...
        if args:
            final.append(args)
        kwds = ", ".join(
            [("%s=%r") % (k, v) for k, v in enumsort(list(self.kwds.items()))]
        )
        if kwds:
            final.append(kwds)
    ...
    def __setitem__(self, key, value):
        ...
        if _is_internal_class(self._cls_name, value):
            ...
            if key not in (
                ...
                allowed_settings = dict.fromkeys(
                    ["autovalue", "autonumber", "noalias", "unique", "multivalue"]
                )
                for arg in value:
                    if arg not in allowed_settings:
                        raise TypeError(
                            "unknown qualifier: %r (from %r)" % (arg, value)
                        )
                ...
                if isinstance(value, staticmethod):
                    ...
                    raise TypeError(
                        "_generate_next_value should be a staticmethod, not a classmethod"
                    )
                else:
                    gnv = value
            ...
            if self._multivalue:
                ...
                if isinstance(value, int):
                    ...
                    if self._init is None:
                        ...
                        "value" not in self._init and len(value) == len(self._init) + 1
                    ):
                        ...
                        # mismatch
                        raise TypeError(
                            "%s: number of fields provided do not match init" % key
                        )
                else:
                    if self._init is not None and (
                        len(self._init) != 1 or "value" in self._init
                    ):
                        raise TypeError(
                            "%s: number of fields provided do not match init" % key
                        )
...
class EnumMeta(StdlibEnumMeta or type):
    ...
    def __prepare__(metacls, cls, bases, init=None, start=None, settings=()):
        ...
        if init is not None:
            ...
            Flag is not None
            and any(issubclass(b, Flag) for b in bases)
            and member_type not in (int, object)
        ):
    ...
    def __new__(metacls, cls, bases, clsdict, init=None, start=None, settings=()):
        ...
        if type(clsdict) is not _EnumDict:
            ...
            _create_pseudo_member_values_ = clsdict.pop(
                "_create_pseudo_member_values_", None
            )
            ...
            original_dict = clsdict
            clsdict = metacls.__prepare__(
                cls, bases, init=init, start=start, settings=settings
            )
            ...
            if isinstance(original_dict, OrderedDict):
                ...
                calced_order = [
                    name for (name, value) in enumsort(list(enum_members.items()))
                ]
                ...
                if init:
                    if not isinstance(init, basestring):
                        init = " ".join(init)
                member = NamedTuple(
                    "member", init and "name " + init or ["name", "value"]
                )
                calced_order = []
                for name, value in enum_members.items():
                    ...
                    if member._defined_len_ != len(name_value):
                        raise TypeError(
                            "%d values expected (%s), %d received (%s)"
                            % (
        ...
        for member_name in clsdict._member_names:
            ...
            if noalias:
                ...
                if nonunique:
                    ...
                    raise ValueError(
                        "duplicate names found in %r: %s" % (cls, ";  ".join(message))
                    )
        ...
        if _order_:
            ...
            if _order_ != enum_class._member_names_:
                raise TypeError(
                    "member order does not match _order_: %r %r"
...
def __init__(self, *args, **kwds):
    ...
    if _auto_init_:
        if len(_auto_init_) < len(args):
            raise TypeError(
                "%d arguments expected (%s), %d received (%s)"
            ...
            for canonical_value in _values_:
                if canonical_value == new_member._value_:
                    ...
                    if _unique_ or _multi_value_:
                        ...
                        raise ValueError(
                            "%s is a duplicate of %s" % (name, canonical_member.name)
                        )
    ...
    def __init__(self, cls, *args):
        ...
        if not args:
            args = [
                ...
                if isinstance(v, (NamedConstant, Enum))
            ]
        for name in args:
